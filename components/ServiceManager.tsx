        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className={`flex justify-between items-end p-6 rounded-xl border shadow-sm transition-colors ${isBlackAndWhite ? '!bg-slate-900 !border-slate-800' : 'bg-white border-gray-200'}`}>
              <div>
                  ...
                  <p className={`mt-1 ${isBlackAndWhite ? 'text-gray-400' : 'text-gray-500'}`}>Nöbet noktalarını ve kurallarını yapılandırın.</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {services.map(s => (
                <Card key={s.id} className={`p-5 relative group hover:shadow-lg transition-all border-l-4 ${isBlackAndWhite ? '!bg-slate-900 !border-slate-800 border-l-slate-600' : (s.isEmergency ? 'border-l-rose-500' : 'border-l-indigo-500')}`}>
                  ...
                  <div className={`space-y-2 text-sm mt-4 p-3 rounded-lg ${isBlackAndWhite ? '!bg-slate-800 text-gray-300' : 'bg-gray-50 text-gray-600'}`}>
